...
nodo_abb_t *insertar_rec(nodo_abb_t *raiz, void *elemento,
...
abb_t *abb_insertar(abb_t *arbol, void *elemento)
{
	...
	arbol->nodo_raiz =
		insertar_rec(arbol->nodo_raiz, elemento, arbol->comparador);
	arbol->tamanio++;
...
nodo_abb_t *quitar_rec(abb_t *arbol, nodo_abb_t *raiz, void *elemento,
		       bool *se_elimino)
{
	if (!raiz)
		return NULL;
		...
		if (!raiz->derecha || !raiz->izquierda) {
			...
			*se_elimino = true;
			...
			return nuevo_hijo;
			   ...
			   raiz->izquierda) { // Caso 2: con 2 hijos
			void *elemento_aux = NULL;
			raiz->izquierda = buscar_predecesor(raiz->izquierda,
							    &elemento_aux);
	...
	if (comparacion < 0) {
		raiz->izquierda = quitar_rec(arbol, raiz->izquierda, elemento,
					     se_elimino);
	} else if (comparacion > 0) {
		raiz->derecha =
			quitar_rec(arbol, raiz->derecha, elemento, se_elimino);
	}
	return raiz;
}
...
void *abb_quitar(abb_t *arbol, void *elemento)
{
	...
	arbol->nodo_raiz =
		quitar_rec(arbol, arbol->nodo_raiz, elemento, &se_elimino);
...
void *buscar_rec(nodo_abb_t *raiz, void *elemento,
		 int (*comparador)(void *, void *))
{
	if (!raiz)
		return NULL;
...
void *abb_buscar(abb_t *arbol, void *elemento)
{
	...
	void *elemento_buscado =
		buscar_rec(arbol->nodo_raiz, elemento, arbol->comparador);
	if (!elemento_buscado)
		return NULL;
...
size_t abb_recorrer_preorden(nodo_abb_t *raiz, void **array,
			     size_t tamanio_array, size_t *contador)
{
	...
	*contador = abb_recorrer_preorden(raiz->izquierda, array, tamanio_array,
					  contador);
	*contador = abb_recorrer_preorden(raiz->derecha, array, tamanio_array,
					  contador);
	return *contador;
}
			...
			size_t *contador)
{
	...
	*contador = recorrer_inorden(raiz->izquierda, array, tamanio_array,
				     contador);
	if (*contador < tamanio_array) {
		array[(*contador)++] = raiz->elemento;
	}
			  ...
			  size_t *contador)
{
	...
	*contador = recorrer_postorden(raiz->izquierda, array, tamanio_array,
				       contador);
	*contador = recorrer_postorden(raiz->derecha, array, tamanio_array,
				       contador);
				    ...
				    size_t *contador)
{
	...
	continuar = abb_con_cada_elemento_preorden(raiz->derecha, funcion, aux,
						   contador);
	if (!continuar)
		return false;
				   ...
				   size_t *contador)
{
	...
	continuar = abb_con_cada_elemento_inorden(raiz->derecha, funcion, aux,
						  contador);
	if (!continuar)
		return false;
				     ...
				     size_t *contador)
{
	...
	continuar = abb_con_cada_elemento_postorden(raiz->derecha, funcion, aux,
						    contador);
	if (!continuar)
		return false;
		    ...
		    size_t tamanio_array)
{
	...
	switch (recorrido) {
	case PREORDEN:
		contador = abb_recorrer_preorden(arbol->nodo_raiz, array,
						 tamanio_array, &contador);
		break;
	case INORDEN:
		contador = recorrer_inorden(arbol->nodo_raiz, array,
					    tamanio_array, &contador);
		break;
	case POSTORDEN:
		contador = recorrer_postorden(arbol->nodo_raiz, array,
					      tamanio_array, &contador);
		break;
	default:
		break;
